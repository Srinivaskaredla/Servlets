...
List<User> retailersList = adminService.viewAllRetailers(); // Use your service method to fetch users
request.setAttribute("retailers", retailersList); // Set the list as an attribute
%>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Admin Dashboard - All Retailers</title>
...
<body>
	...
	<main>
		<div class="container">
			<h1 class="display-4">List of Retailers</h1>
			<table>
				<thead>
					<tr>
						<th>Name</th>
				...
				<tbody>
					...
					if (retailers != null && !retailers.isEmpty()) {
						for (User retailer : retailers) {
							boolean isBlocked = retailer.isBlocked();
					%>
					<tr>
						...
						<td>
							...
							if (isBlocked) {
							%> <a
							href="AdminServlet?taskType=unblockUser&email=<%=retailer.getEmail()%>"
							class="btn btn-success btn-sm">Unblock</a> <%
 } else {
 %> <a
							href="AdminServlet?taskType=blockUser&email=<%=retailer.getEmail()%>"
							class="btn btn-danger btn-sm">Block</a> <%
 }
 %> <a
							href="AdminServlet?taskType=deleteUser&email=<%=retailer.getEmail()%>"
